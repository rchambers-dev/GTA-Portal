...
Canonical definitions live in `src/features/apprentice-lifecycle/domain/adm14-checklist.ts` (sections 1–8, M/O, portal visibility). Apprentice and employer each get a Documents hub plus one page per booklet section; individual document UIs are wired as GTA paper forms are supplied.
