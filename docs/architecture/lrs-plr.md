...
1. Apprentice identity captured on AF1.1 (given name, family name, DOB, gender, ULN)
2. Portal finds ULN if missing (LRS Find ULN)
3. Portal requests **Get Apprentice Learning Events** for that learner
...
- GTA must be registered as a Apprentice Registration Body and approved for Get Apprentice Learning Events
...
- Apprentice must verify and PLR-sharing / privacy status must allow access
...
| Piece | Path |
|-------|------|
| Port | `src/features/apprentice-portal/ports/lrs-plr.ts` |
| Mock + SOAP stub | `src/adapters/lrs/lrs-plr-adapter.ts` |
| Store / auto-fetch | `src/features/apprentice-portal/domain/plr-store.ts` |
| Documents UI | `src/features/apprentice-portal/components/PlrReportPanel.tsx` |
