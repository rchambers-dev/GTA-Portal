...
- Official practical IDs: **Task 3 / Task 4** (filenames).
- Job cards are **not** OTJ (work evidence, not training).
- **Task 5 reflection** mandatory before next block; gate = apprentice → mentor → **trainer verifies**.
- Apprentices: **tasks only** (no lesson plans). Staff/tutors: **lesson plans + tasks**.
- Employers sign mentor items **in the employer portal** (paperless).

## Submission

- Preferred: **portal form**.
- Fallback: **PDF upload** for that college day — **all** required PDFs, each mapped to its own task.
- One active method per task attempt; sign-off sits on the task record.
- Difficulty feedback on every completed task (adapt curriculum next year).
