...
import { LifecycleBoardScreen } from "@/features/apprentice-lifecycle";
...
export default async function ApprenticeLifecyclePage({ searchParams }: Props) {
  const ports = getStandalonePorts();
  const session = await ports.auth.getEffectiveSession();
  if (!session) {
    redirect(getUnauthenticatedRedirect("/apprentices/lifecycle"));
  }
  assertRouteAccess(session, "/apprentices/lifecycle");
