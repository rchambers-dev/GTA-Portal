...
import { CHAT_SELF_APPRENTICE, defaultGroupTitle } from "../domain/chat/store";
import type { ChatChannelType, ChatContact, ChatThread } from "../domain/chat/types";
import styles from "./ApprenticeMessagesScreen.module.css";
...
export function ApprenticeMessagesScreen({
  ...
  function threadUnreadCount(thread: ChatThread): number {
    if (selfContactId === CHAT_SELF_APPRENTICE) return thread.unreadForApprentice;
    const last = thread.messages[thread.messages.length - 1];
