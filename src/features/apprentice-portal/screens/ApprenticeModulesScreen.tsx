...
function statusLabel(status: ApprenticeModuleRow["status"]) {
  switch (status) {
    case "completed":
      return "Completed";
    case "in_progress":
...
function ModuleRow({ mod }: { mod: ApprenticeModuleRow }) {
    ...
    <>
      ...
      <div className={styles.rowEnd}>
        {mod.released ? (
          ...
          <>
            <ApprenticeStatusChip tone="neutral">Not released</ApprenticeStatusChip>
  ...
  return (
    <Link href={`/apprentice/modules/${mod.id}`} className={styles.rowLink}>
      {body}
    </Link>
  );
}
