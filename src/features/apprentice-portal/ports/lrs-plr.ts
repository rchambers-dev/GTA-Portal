 ...
 * not a public REST API. GTA must be approved for Get Apprentice Learning Events.
 */

export type LrsGender = "M" | "F" | "U" | "";

export type LrsApprenticeIdentity = {
  uln: string;
  givenName: string;
  familyName: string;
  dateOfBirth?: string;
...
export interface LrsPlrPort {
  ...
  findUln(identity: Omit<LrsApprenticeIdentity, "uln"> & { uln?: string }): Promise<LrsFindUlnResult>;
  getApprenticeLearningEvents(identity: LrsApprenticeIdentity): Promise<LrsGetPlrResult>;
}
