...
export type PlrStoreState = {
  identity: LrsApprenticeIdentity | null;
...
export function identityReadyKey(identity: LrsApprenticeIdentity | null): string {
  if (!identity) return "";
  return [
    identity.uln,
...
export function isIdentityFetchReady(identity: LrsApprenticeIdentity | null): boolean {
...
export async function refreshPlrFromLrs(options?: {
  ...
  if (!isIdentityFetchReady(identity)) {
    const next = {
      ...
      status: "missing_identity" as const,
      lastError:
        "ULN (10 digits), given name and family name are required for LRS Get Apprentice Learning Events.",
    };
    setMemory(next);
    return next;
  }
  ...
  const result = await port.getApprenticeLearningEvents(identity);
