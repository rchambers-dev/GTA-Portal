...
function partyLabel(role: "Learner" | "Employer" | "Teacher", status: OtjPartyStatus | "done") {
  if (role === "Learner" && status === "done") return "Learner submitted";
  switch (status) {
    case "agreed":
      return `${role} agreed`;
    case "returned":
...
function pipelineTone(entry: LearnerOtjEntry) {
...
function OtjApprovalTrail({ entry }: { entry: LearnerOtjEntry }) {
  const learnerDone = entry.learnerConfirmed;
  ...
  return (
    <ol className={styles.otjTrail} aria-label="Approval order">
      <li
        ...
        data-state={learnerDone ? "done" : "pending"}
      >
        <span className={styles.otjTrailNum}>1</span>
        <span>
          <strong>Student</strong>
          <span>{learnerDone ? "Submitted" : "Not submitted"}</span>
        </span>
      </li>
    ...
    >
      ...
      >
        ...
        <span className={styles.otjEntryMeta}>
          ...
          <LearnerStatusChip tone={pipelineTone(entry)}>
        ...
        <div id={panelId} className={styles.otjEntryPanel}>
          ...
          <div className={styles.otjChipRow}>
            <LearnerStatusChip tone="green">
              {partyLabel("Learner", "done")}
            </LearnerStatusChip>
            <LearnerStatusChip tone={partyTone(entry.employerStatus)}>
              {partyLabel("Employer", entry.employerStatus)}
            </LearnerStatusChip>
            <LearnerStatusChip tone={partyTone(entry.tutorStatus)}>
          ...
          {showForm ? (
            ...
            >
              ...
              <label className={styles.otjConfirm}>
                <input
                  type="checkbox"
                  checked={learnerConfirmed}
                  onChange={(e) => setLearnerConfirmed(e.target.checked)}
                />
                <span>
