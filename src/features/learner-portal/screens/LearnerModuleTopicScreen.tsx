...
export function LearnerModuleTopicScreen({
  moduleId,
  topicId,
}: {
  moduleId: string;
  ...
  if (!result) {
    return (
      <LearnerPageShell
        ...
        actions={
          <StepBackButton parentHref={`/learner/modules/${moduleId}`} />
        }
      >
  ...
  return (
    <LearnerPageShell
      ...
      actions={
        <>
          <LearnerStatusChip tone={topicTone(topic.status)}>
            {topicLabel(topic.status)}
          </LearnerStatusChip>
          <StepBackButton
            parentHref={`/learner/modules/${moduleId}`}
            label="Back"
          />
        </>
      }
    >
      <nav className={styles.crumb} aria-label="Breadcrumb">
        ...
        <Link href={`/learner/modules/${moduleId}`}>
          {module.code}
        </Link>
        <span aria-hidden>/</span>
