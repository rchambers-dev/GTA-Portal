...
export type AgreementAudience = "learner" | "employer" | "mentor";
...
export const TRAINING_PLAN_APPRENTICE_SECTIONS: AgreementSection[] = [
  {
    ...
    items: [
      "Provide relevant information to support learner and programme eligibility checks.",
...
export function readTrainingPlanAgreement(): TrainingPlanAgreementState {
  ...
  try {
    ...
    return {
      learner: isSignature(parsed.learner)
        ? parsed.learner
        : DEFAULT_TRAINING_PLAN_AGREEMENT.learner,
...
export function useTrainingPlanAgreementState() {
  ...
  const summary = useMemo(
    () => ({
      allSigned: state.learner.signed && state.employer.signed && state.mentor.signed,
      learnerPending: !state.learner.signed,
