 ...
 * learners add those themselves or via AI.
 */
...
export type CvPortalSeed = {
  ...
  houseNumber: string;
  postcode: string;
  /** Full looked-up address line selected by the learner (optional). */
  addressLine: string;
  summary: string;
  skills: string[];
  experience: Array<{
  ...
  /** Preset line on the CV — learner can remove it. */
...
function toPublicModule(row: LearnerModuleRow): CvModulePublic {
  return {
    code: row.code,
    title: row.title,
    year: row.year,
 ...
 * Ready for live portal data — swap ALEX_MODULES for the signed-in learner's
 * modules when that feed is wired; shape stays the same.
 */
export function getCvModulesPublic(
  modules: LearnerModuleRow[] = ALEX_MODULES,
): CvModulePublic[] {
  return modules
...
function completedModuleTitles(modules: LearnerModuleRow[] = ALEX_MODULES): string[] {
  return modules.filter((m) => m.status === "completed").map((m) => m.title);
}

function educationDetail(
  profile: LearnerPortalProfile,
  modules: LearnerModuleRow[] = ALEX_MODULES,
): string {
  const parts = [
 ...
 * Summary, skills, contact extras, and references stay blank for the learner.
 */
export function buildCvFromPortal(
  profile: LearnerPortalProfile = ALEX_PROFILE,
  modules: LearnerModuleRow[] = ALEX_MODULES,
): CvPortalSeed {
