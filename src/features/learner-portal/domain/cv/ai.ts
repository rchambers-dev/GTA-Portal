 ...
 * Uses getCvModulesPublic() so live learner modules can replace the mock
...
function baseContext(draft: CvDraftForAi): PortalAiContextBag[] {
  return [
    ...
    { scope: "learner.profile_public", data: publicProfile() },
    { scope: "learner.programme_public", data: publicProgramme() },
  ];
}
  ...
  const result = await requestPortalAi({
    ...
    context: [
      ...
      ...(modules.modules.length > 0
        ? [{ scope: "learner.modules_public" as const, data: modules }]
        : []),
    ],
  });
...
export async function aiSuggestBulletsFromModules(
  ...
  const result = await requestPortalAi({
    ...
    context: [
      ...baseContext(focused),
      { scope: "learner.modules_public", data: modules },
    ],
  });
