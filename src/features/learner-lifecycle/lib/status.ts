...
export function overallStatusLabel(status: OverallLearnerStatus): string {
  const labels: Record<OverallLearnerStatus, string> = {
    pre_start: "Pre-start",
    on_track: "On track",
...
export function overallStatusTone(status: OverallLearnerStatus): StatusTone {
  switch (status) {
    case "on_track":
    case "completed":
      return "on_track";
