...
function recordFromRow(
  ...
  const stored = getPackItem(apprenticeId, row.reference);
  if (stored) {
    return {
      ...stored,
      notApplicable:
...
export function PackItemEditor({ apprenticeId, row, onClose, onSaved }: Props) {
  ...
  function save() {
    ...
    upsertPackItem(apprenticeId, row.reference, next);
    setDraft(next);
    setSavedFlash(true);
    onSaved();
        ...
        <div className={styles.body}>
          ...
          {row.requirementKind === "conditional" ? (
            <label className={styles.checkRow}>
              ...
              />
              <span>Does not apply for this apprentice</span>
            </label>
          ) : null}
